...
OpenLayers.Control.ModifyFeature = OpenLayers.Class(OpenLayers.Control, {
    ...
    /**
     * Property: pointStyle
     * Layer style applied to current selected point feature, internal use.
     */
    pointStyle: null,
        ...
        if (toolName === 'point') {
            ...
            if (vertexStyle.fillOpacity === 0 &&
                ...
                if (!pointStyle.externalGraphic) {
                    OpenLayers.Util.extend(vertexStyle, {
                        pointRadius: pointStyle.pointRadius ||
                                                        vertexStyle.pointRadius,
                        rotation: pointStyle.rotation || vertexStyle.rotation,
                        graphicName: pointStyle.graphicName ||
                                                        vertexStyle.graphicName
                    });
                }
            }
        }
     ...
     * Called when the select feature control unselects the current selected
     *     feature.
     */
    ...
    resetVertices: function(onlyToolbar) {
        ...
        if (this.feature) {
            ...
            var yOffset = 0,
                feature = this.feature,
                layer = feature.layer,
                pointStyle = layer.styleMap.createSymbolizer(
                                                         feature, 'select');
    ...
    collectVertices: function(mode) {
        ...
        var _pushPoint = function(geometry) {
            ...
            _virtualVertices.push(control.createVertex('point', false, p));
        };
    ...
    collectTools: function(mode, onlyToolbar, yOffset) {
        ...
        var center = bounds.getCenterLonLat(),
            ...
            _posX = bounds.right,
            _posY = bounds.top + (yOffset ?
                                    _sizeY / 2 + yOffset * resolution :
                                    _sizeY),
            addToToolbar = function(toolName, pressingAction, p_dxAction) {
        ...
        if (!isPoint) {
            if (!onlyToolbar && mode & MODES.DRAG) {
                ...
                this.layer.addFeatures(_dragTools, {silent: true});
        ...
        this.layer.addFeatures(_toolbar, {silent: true});
